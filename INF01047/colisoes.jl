...
function Plots.plot!(circle::Circle)
	step = 2π/num_samples
	range = 0:(2π/num_samples):(2π+step)
	plot!(
		cos.(range) .* circle.radius .+ circle.pos[1],
		sin.(range) .* circle.radius .+ circle.pos[2],
		color=circle.color, linewidth=linewidth
	)
end

# ╔═╡ 5a7f2fad-47fc-4527-9f02-725b6e74074a
function Plots.plot!(box::Box)
	left = box.pos[1] - (box.x_side) / 2
	right = box.pos[1] + (box.x_side) / 2
	bottom = box.pos[2] + (box.y_side) / 2
	top = box.pos[2] - (box.y_side) / 2
	plot!([left, right], [bottom, bottom], color=box.color, linewidth=linewidth)
	plot!([left, right], [top, top], color=box.color, linewidth=linewidth)
	plot!([left, left], [top, bottom], color=box.color, linewidth=linewidth)
...
function Plots.plot!(state::State)
	p = nothing
