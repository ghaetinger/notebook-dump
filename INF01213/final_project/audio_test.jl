...
# ╔═╡ 76c121be-1fcf-11ed-1c9b-f795e0b21561
using FrequencyAdjustment, Images, TestImages, WAV, Plots

# ╔═╡ 02d2397c-d9f4-4833-8d96-415aca1645c8
s = Gray{Float64}.(testimage("barbara_color"))

# ╔═╡ d1b15897-db26-400d-91ca-0aaa63e2994d
detection_parameters = (;
    R = 4, # Expected image downscaling factor
    σ = 3, # Gaussian window standard deviation, in pixels
)

# ╔═╡ d0685ebe-69e8-493e-b169-5158463238e0
# s_0p5 = adjust(s, 0.5; detection_parameters...)

# ╔═╡ bf684cba-d9fc-4a62-b594-7f0e52b6df88
s_0p1 = adjust(s, 0.1; detection_parameters...)

# ╔═╡ 9266900b-5802-406e-a30f-873da84fd773
begin
   fs = 8e3
...
# ╔═╡ 07955b06-8681-4393-8891-3964a12db07a
function signal_to_image(signal; n_rows=5)
    repeated_signal = repeat(signal, n_rows)
    (lb, ub) = extrema(repeated_signal)
    normalized = (repeated_signal .- lb) ./ (ub - lb)
    reshaped = reshape(normalized, (length(signal), n_rows))
    return Gray{Float64}.(transpose(reshaped))
end

# ╔═╡ 2edbaf2e-0ee6-47e7-992a-b12723e3f6e6
signal_img = signal_to_image(y; n_rows=10);

# ╔═╡ 292266c8-5155-49d0-a839-c3a51969a913
new_signal_image = adjust(signal_img, 0.1; detection_parameters...);

# ╔═╡ bba1e447-368d-485a-b4eb-290405ea1ecf
new_signal = Float64.(new_signal_image[1, 1:end])

# ╔═╡ 8beae1e7-e52f-4292-8c5d-cf75febcf9fc
wavwrite(new_signal, "example-0.1.wav", Fs=fs)

# ╔═╡ a4acb5f5-4740-48dc-8ed9-8db7c43004c9
prev_signal = Float64.(signal_img[1, 1:end])

# ╔═╡ 154f1af5-6d9f-41b7-84a5-c82942709a66
wavwrite(y, "example_before.wav", Fs=fs)

# ╔═╡ 00000000-0000-0000-0000-000000000001
PLUTO_PROJECT_TOML_CONTENTS = """
[deps]
FrequencyAdjustment = "890ad22b-9db7-4274-a573-e31dbb1ee613"
...
[[deps.FrequencyAdjustment]]
deps = ["AbstractFFTs", "ColorTypes", "Combinatorics", "DSP", "FFTW", "Images", "Lazy", "LinearAlgebra", "MappedArrays", "OffsetArrays", "Printf", "ProgressLogging", "Requires", "SparseArrays", "Statistics", "StatsBase"]
git-tree-sha1 = "e95de9e624d09d5dea3120b16590d006039ee777"
uuid = "890ad22b-9db7-4274-a573-e31dbb1ee613"
version = "0.1.0"
